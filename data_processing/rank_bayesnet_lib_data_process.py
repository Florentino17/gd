...
from util.date_utils.date_function import is_weekday_commit \
    , project_age, get_close_pr_time,get_waiting_time
...
def text_save(all_filename, train_filename, test_filename, data, headers):  # filename为写入CSV文件的路径，data为要写入数据列表.
    ...
    train_file.close()
    test_file.close()
    print("【rank_bayesnet_lib_data_process.py】保存文件成功")
...
def get_data_by_repo_name(repo_name):
    ...
    print("【rank_bayesnet_lib_data_process.py】len(raw_data)=",len(raw_data))  ##查看PR数量
    ...
    print("【rank_bayesnet_lib_data_process.py】count=",count)
    ...
    for i in range(len(X_successive)):
        # label_dict的键就是pr_id，不是pr_number 所以取值可以用0,1,2,3类似下标的方式取值
    ...
    for i in range(len(X_dispersed)):
        ...
        for j in X_successive[i]:
            tmp.append(j)
        X.append((X_dispersed[i][0], tmp))  #X_dispersed[i][0]是pr_number
    X_dict = dict(X)
    ##获取每个PR的响应时间
    pr_temp = []
    merge_result_dict={}
    ...
    for item in process_data:
        ...
        tmp.append(('pr_user_name', item[pr_user_name_index]))
        tmp = dict(tmp)
        pr_temp.append((item[pr_number_index], tmp))
        merge_result_dict[item[pr_number_index]] = int(item[merged_index])*4

    pr_temp = dict(pr_temp)
    # 获取pr从创建到关闭的时间
    first_response_time_dict = get_waiting_time(pr_temp)  # get_waiting_time(first_response_time)
    close_time_dict=get_close_pr_time(pr_temp)
    ##响应时间
    first_response_time=[]
    close_time=[]
    for item in first_response_time_dict.keys():
        first_response_time.append(first_response_time_dict[item])
    for item in close_time_dict.keys():
        close_time.append(close_time_dict[item])
    
    true_response_rate_label_dict = get_true_order_dict(first_response_time, first_response_time_dict)
    true_close_rate_label_dict = get_true_order_dict(close_time, close_time_dict)
    true_rate_label_dict = {}
    for key in merge_result_dict.keys():
        true_rate_label_dict[key] = (merge_result_dict[key]+true_response_rate_label_dict[key]+true_close_rate_label_dict[key])/3
...
if __name__ == '__main__':
    repo_name =get_repo_name()#"scikit-learn"#"moby"#"cocos2d-x"#"netbeans"#"yii2"#"dubbo"#"react"#"tensorflow"#"opencv"#"phoenix"#"helix"#"terraform"#"Ipython"#"kuma"#"incubator-heron"# "kuma"#"Katello"#"zipkin"#"yii2"
