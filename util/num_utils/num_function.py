        ...
        if temp_dict.__len__() == 0:
            ...
            for temp_key in temp_dict.keys():
                ...
                if temp_created_time < created_time and temp_closed_time > created_time:
                    count = count + 1
                else: #这个pr的关闭时间小于以后所有PR的创建时间，可以移除
                    remove_list.append(temp_key)
            for temp_key in remove_list:#移除不再需要的PR
                temp_dict.pop(temp_key)
            re_dict[key] = count
            temp_dict[key] = pr_dict[key] #将当前pr添加到temp_dict中
    return re_dict
...
def get_change_num(pr_change_dict):
    """
       获取该pr提交之前，提交者在该代码仓库中提交的pr中的代码更改总数
       user_pr_dict
       {
    ...
    for key in pr_change_dict.keys():
        ...
        if temp_dict.__len__() > 0:
            for temp_key in temp_dict.keys():
                ...
                if temp_user_name == pr_user_name:
                    # print("count=",count)
                    # print("temp_dict[temp_key]['changed_line_num']=",temp_dict[temp_key]['changed_line_num'])
                    # count = count + temp_dict[temp_key]['changed_line_num']
                    count = count + (int)(temp_dict[temp_key]['changed_line_num'])
...
def get_accept_num(pr_dict):
    """
       获取该pr提交之前，提交者在该代码仓库中提交的pr的接受总数
       user_pr_dict
       {
...
def get_review_num(pr_dict):
    """
     该pr作者在该代码仓库之前评审过多少pr review_comments
       pr_dict
       {
           52949: {
            'pr_user_name': 'tlemo',
    ...
    for key in pr_dict.keys():
        ...
        try:
            ...
            print("【util/num_utils/num_function】",str(key) + "      ")
            print("【util/num_utils/num_function】",e)
            break
    return re_dict
