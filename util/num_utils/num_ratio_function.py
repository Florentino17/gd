...
def get_pr_author_rate(pr_dict):
    """
       计算pr作者在代码仓的总提交成功率,被关闭的概率，作者提交pr占代码仓总pr的比例，代码仓对所有pr的接受率
       labels_dict
       {
            52948: {
       ...
       转为json后计算json的长度
       如果该pr_author还未提交过，我们认为该pr_user_name的接受概率为1，pr被关闭的概率为1
       返回值
       {
             52949: {
    ...
    for key in pr_dict.keys():
        ...
        if temp_dict.__len__() == 0:
            ...
            if self_pr_num == 0:
                re_dict[key]['self_accept_rate'] = 1
                re_dict[key]['self_closed_num_rate'] = 1
            else:
        ...
        for temp_year in temp_dict.keys():
            for temp_week in temp_dict[temp_year].keys():
                if temp_year == year and temp_week == week:
                    ...
                    temp_add_line = temp_add_line + (int)(temp_dict[temp_year][temp_week]["total_add_line"])
                    temp_delete_line = temp_delete_line + (int)(temp_dict[temp_year][temp_week]["total_delete_line"])
                    temp_change_line = temp_change_line + (int)(temp_dict[temp_year][temp_week]["total_change_line"])
        re_dict[key] = {}
        if contain_week == 0:
...
def get_avg_latency(pr_dict):
    ...
    for key in pr_dict.keys():
        ...
        # 如果close_day=0，表示他在这个数据仓库中还未关闭，后面肯定不会满足
        # temp_closed_time <= created_time这个条件，所以不会参与计算，同理merge_day=0也不会参与计算       
        re_dict[key] = {}
        if temp_dict.__len__() == 0:
