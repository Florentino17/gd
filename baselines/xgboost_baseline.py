...
def get_data_by_repo_name_and_origin_data_path(origin_data_path, repo_name):
    ...
    print("【xgboost_baseline.py】len(data)=",len(data))  ##查看PR数量
    # 获取所在文件中的prlist
...
def prepare_temp_file(temp_data_path, origin_data_path, open_pr_index_list, day):
    ...
    for i in range(len(open_pr_index_list)):
        ...
        file.write(s)
    file.close()
    print("【xgboost_baseline.py】保存第 " + str(day) + " 天的临时文件成功")
...
def alg_model_result(true_rate_label_dict, day_data, pr_number_index_dict, origin_data_path, temp_data_path,
    ...
    for day in day_data.keys():
        print("【xgboost_baseline.py】=================================日期:", day)
        ...
        if sort_result.__len__() == 0:
            print("【xgboost_baseline.py】在" + origin_data_path + "无相关pr")
            continue
        rank_sort = []
        true_sort = []
        # 获取从fifo中获取的每个列表的顺序
        ...
        print("【xgboost_baseline.py】pr_number排序:", sort_result)
        print("【xgboost_baseline.py】rank_sort:", rank_sort)
        print("【xgboost_baseline.py】true_sort:", true_sort)
        print("【xgboost_baseline.py】ndcg_num:", ndcg_num)
        print("【xgboost_baseline.py】mrr_num:", mrr_num)
        print("【xgboost_baseline.py】kendall_num:", kendall_num)
    ...
    for i in range(len(day_list)):
        ...
        row_data.append(tmp)
    print("【xgboost_baseline.py】",row_data)
    # 保存数据到csv文件
...
def train_model(alg_name, rank_type, model_path, train_data_path, train_data_group_path, test_data_path):
    print("【xgboost_baseline.py】===============训练模型+" + alg_name + "======================")
...
if __name__ == '__main__':
    ...
    xgboost_file_path = "./data/xgboost_data/" + repo_name + "/"
    ...
    xgboost_model_path = "./model/xgboost_model/" + repo_name + "/"
    ...
    print("【xgboost_baseline.py】alg_name=",alg_name + "模型训练完成==========")
