...
SJF算法，根据pr的大小（修改行数）排序，尺寸最小的放在最前面，这样对上述pr列表进行排序。SJF_Y
真实排序：，根据pr被关闭的时间顺序进行排序，进而获取真实排序TRUE_Y
将SJF_Y，与TRUE_Y进行比较，通过NDCG进行比较，判断排序效果
'''
import sys
import os
...
from util.date_utils.date_function import get_waiting_time,get_close_pr_time
import csv
...
def get_data_by_repo_name(repo_name):
    ...
    print("【short_job_first.py】len(data)=",len(data))  ##查看PR数量
    ...
    return day_data, merge_result_dict,response_time, first_response_time_dict, close_time, close_time_dict


# print(response_time)
# 根据已有数据，得到修改行数最小的PR
def sjf(day_data, day):
    sjf_data = []
    ...
    for i in range(change_line.__len__()):
        ...
        for pr_key in temp_key_line:
            if temp_key_line[pr_key] == temp_lines and sjf_data.__contains__(pr_key) is False:
                sjf_data.append(pr_key)
                break
    return sjf_data


# 对sjf进行调用，同时将数据写入到文件中，方便后续统计
def sjf_result(true_rate_label_dict, day_data, repo_name):
    ndcg_list = []
    mrr_list = []
    kendall_list = []
    day_list = []
    ...
    for day in day_data.keys():
        ...
        sjf_data = sjf(day_data, day)
        sjf_sort = []
        true_sort = []
        # 获取从ssf中获取的每个列表的顺序
        for pr_number_sjf in sjf_data:
            sjf_sort.append(true_rate_label_dict[pr_number_sjf])
            true_sort.append(true_rate_label_dict[pr_number_sjf])
        true_sort.sort(reverse=True)
        ndcg_num = ndcg(true_sort, sjf_sort, sjf_sort.__len__(),form = "exp")
        mrr_num = mrr(true_sort, sjf_sort)
        kendall_num = kendall_tau_distance(true_sort, sjf_sort)
        print("【short_job_first.py】=================================日期:", day)
        print("【short_job_first.py】ssf pr_number排序:", sjf_data)
        print("【short_job_first.py】ssf_sort:", sjf_sort)
        print("【short_job_first.py】true_sort:", true_sort)
        print("【short_job_first.py】ndcg_num:", ndcg_num)
        print("【short_job_first.py】mrr_num:", mrr_num)
        print("【short_job_first.py】kendall_num:", kendall_num)
    ...
    for i in range(len(day_list)):
        ...
        row_data.append(tmp)
    print("【short_job_first.py】row_data=",row_data)
    # 保存数据到csv文件
    file_path = "./result/short_job_first/"
    path_exists_or_create(file_path)
    with open(file_path + repo_name + "_short_job_first_result.csv", 'w', encoding='utf-8', newline='') as f:
...
if __name__ == '__main__':
    repo_name =get_repo_name()#"scikit-learn"#"moby"#"cocos2d-x"#"netbeans"#"yii2"#"dubbo"#"react"#"tensorflow"#"opencv"#"phoenix"#"helix"#"terraform"#"Ipython"#"kuma"#"incubator-heron"#  "Katello"#"zipkin"#"kuma"
    day_data, merge_dict,response_time, first_response_time_dict,close_time,close_time_dict = get_data_by_repo_name(repo_name)
    ...
    sjf_result(true_rate_label_dict, day_data, repo_name)
