...
from util.print_photo import showBN
...
def get_pr_number_from_origin_data_path(origin_data_path):
    ...
    for i in range(pr_number_series.__len__()):
        print("【bayesian_network_baseline.py】",str(i) + "   pr_number_series   " + str(pr_number_series[i]))
...
def get_data_by_repo_name_and_origin_data_path(origin_data_path, repo_name):
    ...
    print("【bayesian_network_baseline.py】len(data)=",len(data))  ##查看PR数量
    # 获取所在文件中的prlist
...
def model_result(day_data, day, pr_number_index_dict, result_rate_in_dict):
    ...
    if open_pr_list.__len__() == 0:
        return sort_result
    temp_sort_tmp = []
    print("【bayesian_network_baseline.py】文件结果获取计算中======")
...
def alg_model_result(true_rate_label_dict, day_data, pr_number_index_dict, result_rate_in_dict, result_path):
    ...
    for day in day_data.keys():
        print("【bayesian_network_baseline.py】=================================日期:", day)
        ...
        if sort_result.__len__() == 0:
            print("【bayesian_network_baseline.py】在" + origin_data_path + "无相关pr")
            continue
        rank_sort = []
        true_sort = []
        # 获取从fifo中获取的每个列表的顺序
        ...
        print("【bayesian_network_baseline.py】pr_number排序:", sort_result)
        print("【bayesian_network_baseline.py】rank_sort:", rank_sort)
        print("【bayesian_network_baseline.py】true_sort:", true_sort)
        print("【bayesian_network_baseline.py】ndcg_num:", ndcg_num)
        print("【bayesian_network_baseline.py】mrr_num:", mrr_num)
        print("【bayesian_network_baseline.py】kendall_num:", kendall_num)
    ...
    for i in range(len(day_list)):
        ...
        row_data.append(tmp)
    print("【bayesian_network_baseline.py】row_data=",row_data)
    # 保存数据到csv文件
...
def train_model_and_result(alg_name, train_data_path, test_data_path, repo_name):
    module_result = {}
    print("【bayesian_network_baseline.py】===============训练模型+" + alg_name + "======================")

    train = pd.read_csv(train_data_path)
    train_dataset = train.drop(columns=['pr_number'])
    train_dataset.dropna(inplace=True)#去除缺失行，但实际上没有缺失
    train_dataset['priorities_number'] = train_dataset['priorities_number'].astype(float)
    print("【bayesian_network_baseline.py】train_dataset.head()=",train_dataset.head())
    bayesian_model_path = "./model/bayesian_network/" + repo_name + "/"
    ...
    print("【bayesian_network_baseline.py】best_model.edges()=",best_model.edges())
    ...
    for key in best_model.nodes:
        print("【bayesian_network_baseline.py】key=",key)
    ...
    print("【bayesian_network_baseline.py】===============结束训练模型+" + alg_name + "======================")

    print("【bayesian_network_baseline.py】===============开始利用模型计算结果+" + alg_name + "======================")
    test = pd.read_csv(test_data_path)
    test_dataset = test.drop(columns=['pr_number'])
    test_dataset.dropna(inplace=True)
    test_dataset['priorities_number'] = test_dataset['priorities_number'].astype(float)
    predict_data = test_dataset.drop(columns='priorities_number', axis=1)

    print("【bayesian_network_baseline.py】set(predict_data.columns) - set(best_model.nodes())=",set(predict_data.columns) - set(best_model.nodes()))
    ...
    for row_index in range(temp_data.__len__()):
        try:
            # n_jobs=1表示使用一个CPU进行计算
            y_pred_temp = best_model.predict(temp_data.loc[[row_index]], n_jobs=1)
        except Exception as e:
            # 如果发生错误则回滚
            print("【bayesian_network_baseline.py】第", row_index, "行数据预测失败原因是: ", e)
    ...
    print("【bayesian_network_baseline.py】y_pred=",y_pred)
    ...
    for i in range(pr_number_series.__len__()):
        print("【bayesian_network_baseline.py】",str(i) + "   pr_number_series   " + str(pr_number_series[i]))
        print("【bayesian_network_baseline.py】",str(i) + "  y_pred    " + str(y_pred_series[i]))
...
if __name__ == '__main__':
    repo_name =get_repo_name()#"scikit-learn"#"moby"#"cocos2d-x"#"netbeans"#"yii2"##"dubbo"#"react"#"tensorflow"#"opencv"#"phoenix"#"helix"#"terraform"#"Ipython"#"kuma"#"incubator-heron"#"Katello"#"zipkin"#"incubator-heron"
    # ranklib所能调的库
    alg_name = "bayesian_network"
    # 测试模型性能的文件路径
    file_path = "./data/bayesian_data/" + repo_name + "/"
    ...
    print("【bayesian_network_baseline.py】",alg_name + "模型训练完成==========")
    for key in result_rate_in_dict.keys():
        print("【bayesian_network_baseline.py】",str(key) + "....." + str(result_rate_in_dict.get(key)))
