...
def follow_str(url, number, headers):
    ...
    while page <= maxPage:
        ...
        print("【user】temp_url_str=",temp_url_str)
...
'''
# 封装成一个方法，让他方便外部调用
def get_pr_user_info(index, repo_name, headers):
    # 数据操作部分
    # SQL语句书写
    sql = """
        insert into pr_user(
    ...
    while index < data_len:
        ...
        print("【user】 ========================" + "第" + str(index) + "号 user: " + str(
            pr_user_name) + "==========================")
        print("【user】 pr_user_id=",pr_user_id)
        print("【user】 pr_user_name",pr_user_name)
        print("【user】 author_association_with_repo",author_association_with_repo)
        print("【user】 repo_name",repo_name)
        try:
            # 利用游标对象进行操作
            ...
            if pr_user_data.__len__() == 0:
                ...
                print("【user】 temp_pr_url",temp_pr_url)
                temp_pr_url_r = requests.get(temp_pr_url, headers=headers)
                print("【user】 temp_pr_url: " + temp_pr_url + "  Status Code:", temp_pr_url_r.status_code)
                ...
                try:
                    ...
                    # 提交到数据库执行
                    database.commit()
                    print("【user】 第" + str(index) + "号 user: " + str(pr_user_name) + "数据插入成功")

                except Exception as e:
                    # 如果发生错误则回滚
                    print("【user】 第" + str(index) + "号 user: " + str(pr_user_name) + "数据插入数据库失败: " + str(e))
            ...
            # 如果查询到的数据不为空，则说明之前已经爬取过了，则直接更新author_association_with_repo
                ...
                print("【user】 第" + str(index) + "号 user: " + str(pr_user_name) + " 用户信息异常: " + str(pr_user_data.__len__()))
        except Exception as e:
            # 如果发生错误则回滚
            print("【user】 第" + str(index) + "号 user: " + str(pr_user_name) + " 失败: " + str(e))
...
# get_pr_user_info(0,"active_merchant",{'Authorization': 'token ' + get_token()})