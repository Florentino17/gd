...
from util.time_utils import time_reverse
...
def url_to_json(url, number, headers):
    ...
    while page <= maxPage:
        ...
        print("【self】 temp_url_str=",temp_url_str)
...
def get_pr_self_info(index, max_num, owner_name, repo_name, headers):
    ...
    database.ping(reconnect=True) # 防止mysql数据库连接不上

    while index < max_num:
        try:
            print("【self】 ========================" + "第" + str(index) + "号 pr_number: " + str(
                index) + "信息保存中==========================")
            temp_url = pr_url + index.__str__()
            pr_r = requests.get(temp_url, headers=headers)
            print("【self】 pr_url: " + temp_url + "  Status Code:", pr_r.status_code)
        except Exception as e:
            # 如果发生错误则回滚
            print("【self】 网络连接失败: pr_url: ", temp_url)
        ...
        try:
            sqlData = (
                ...
                ((merged == True) and 1 or 0),  #将结果限定在0或者1
                ...
                requested_reviewers_content,
                comments_number,
                json.dumps(comments_content), #将字典转化为json格式字符串
                review_comments_number,
                json.dumps(review_comments_content), #将字典转化为json格式字符串
                commit_number,
                json.dumps(commit_content), #将字典转化为json格式字符串
                changed_file_num,
                total_add_line,
                total_delete_line)
            database.ping(reconnect=True) # 防止mysql数据库连接不上
            ...
            # 提交到数据库执行
            database.commit()
            print("【self】 第", index, "行数据插入数据库成功: ", repo_name)
            index = index + 1
        except Exception as e:
            # 如果发生错误则回滚
            
            print("【self】 第", index, "行数据插入数据库失败: ", "repo_name:", repo_name)
            filename = repo_name + '_exception.csv'

            #如果是重复错误，则跳过 1062即主键重复
...
# get_pr_self_info(0, 50, "activemerchant", "active_merchant", {'Authorization': 'token ' + get_token()})