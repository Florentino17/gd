...
def node_save():
    ...
    #selected_data保存useful_features_index对应的数据
    ...
    for item in selected_data:
        ...
        if item[3]=='CONTRIBUTOR':
            tmp.append(0)   # 贡献者CONTRIBUTOR为0
        elif item[3]=='MEMBER':
            tmp.append(1)   # 成员MEMBER为1
        elif item[3]=='NONE':
            tmp.append(2)   # NONE为2
        else:  
            tmp.append(3)   # 其他为3
    ...
    proj_age=[]
    for item in repo_data:#遍历repo_data，包含五项
        tmp=[]
    ...
    for item in process_data:
        ...
        # print("type(created_at_index)=",type(created_at_index))
    ...
    close_time=dict(first_response_time)

    first_response_time=get_waiting_time(first_response_time)
    close_time=get_close_pr_time(close_time)


    ##是否被合并  (70%)
    Y_1=[]
    for item in process_data:
        Y_1.append(int(item[8])*4)
    
    # 响应速度
    Y_2=[]
    for item in first_response_time.keys():
        Y_2.append(first_response_time[item])
    
    # 关闭速度
    Y_3=[]
    for item in close_time.keys():
        Y_3.append(close_time[item])


    Y=[]
    for i in range(0,len(Y_1)):
        Y.append([Y_1[i],Y_2[i],Y_3[i]])
