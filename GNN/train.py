...
def train_process():
    ...
    response_speed.sort()  #根据响应速度排序
    close_speed.sort()     #根据关闭速度排序

    l=len(y)
    idx=[int(l/5),int(l*2/5),int(l*3/5),int(l*4/5)]
    

    threshold_response_speed=[]  #存放四个值，按照int(l/5),int(l*2/5),int(l*3/5),int(l*4/5)划分的中间的值
    ...
    for i in range(len(response_speed_level)):  #计算真实优先级，除以3
...
# train_process()
    ...
    INPUT_DIM = 44    # 输入维度  44-14
    # Note: 采样的邻居阶数需要与GCN的层数保持一致
    HIDDEN_DIM = [176,88,44]   # 隐藏单元节点数  #20,10,1  #10,10,1  10,10,1 10,10,1  10,10,5,1
