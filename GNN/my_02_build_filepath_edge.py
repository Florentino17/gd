...
def my_02_build_filepath_edge():
    file_path_data=getData.getDataFromSql(
        "select id_in_all,file_name from\
        (select a.id_in_all,file_name,closed from \
        (select id_in_all,file_name  from  pr_all_file where id_in_all<=1000) as a, \
        (select pid,closed          from  pr_all  where  pid<=1000) as b \
            where a.id_in_all=b.pid) as c \
                where c.closed is not null "
    )
    ...
    raw_data=getData.getDataFromSql(
        "select pid,created from pr_all where pid<=1000 and closed is not null order by created asc"
    )
